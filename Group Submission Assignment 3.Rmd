...
bq <- (mean_bq$Bq)
for (i in 1:58) {
  boxplot_data[[i]] <- sapply(bq, function(x) x[i])
}

boxplot_data_df <- data.frame(Value = unlist(boxplot_data), Variable = rep(1:58, each = nrow(boxplot_data)))

# Load ggplot2
library(ggplot2)

# Create the boxplot
ggplot(boxplot_data_df, aes(x = as.factor(Variable), y = Value)) +
  geom_boxplot() +
  labs(x = "Position in Read", y = "Base Quality") +
